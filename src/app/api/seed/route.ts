...
// POST /api/seed — Seed the database with menu items (run once after deploy)
  ...
  try {
    ...
    if (existing > 0) {
      return NextResponse.json({
        message: `Database already has ${existing} menu items. Skipping seed.`,
        seeded: false,
      });
    }
    ...
    const result = await db.menuItem.createMany({ data: menuItems });

    return NextResponse.json({
      message: `Seeded ${result.count} menu items successfully!`,
      seeded: true,
    }, { status: 201 });
  } catch (error) {
    console.error('Seed error:', error);
    return NextResponse.json(
      { error: 'Seed failed. Make sure DATABASE_URL is configured and tables exist (run prisma db push first).' },
      { status: 500 }
    );
  }
}
